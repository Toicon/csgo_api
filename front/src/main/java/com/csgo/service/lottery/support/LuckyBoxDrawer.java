...
public class LuckyBoxDrawer {
    ...
    public LuckyBoxDrawer(LuckyGift gift, UserPlus player, SystemConfigFacade config, BoxJackpotService boxJackpotService, LuckyBoxDrawerContext context) {
        this.gift = gift;
        ...
        this.player = player;
        this.config = config;
        this.boxJackpotService = boxJackpotService;
    ...
    public LuckyGiftProduct newDraw(List<LuckyGiftProduct> products) {
        ...
        for (LuckyGiftProduct product : products) {
            ...
            if (context.mustDrawGold()) {
                ...
                boolean preHit = boxJackpotService.preHit(product.getPrice());
                log.info("[五连出金] 抽奖 productId:{} outProbability:{} price:{} preHit：{}", product.getId(), product.getOutProbability(), product.getPrice(), preHit);
                if (preHit) {
                    isHit = boxJackpotService.hit(product.getPrice(), player, true);
                }
            } else {
                isHit = boxJackpotService.hit(product.getPrice(), player, false);
            }
        ...
        boxJackpotService.hit(product.getPrice(), player, true);
        return product;
    }
    ...
    public void sortAndChangeGiftProduct(List<LuckyGiftProduct> products) {
        ...
        jackpotBalance = boxJackpotService.getBoxJackpotBalance();
        warnValue = config.getWarnValue();
