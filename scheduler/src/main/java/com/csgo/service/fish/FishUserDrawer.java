...
@Slf4j
public class FishUserDrawer {

    private final LuckyGift gift;
    private final UserPlus player;
    private final SystemConfigFacade config;
    private final FishUserJackpotService fishUserJackpotService;
    private final LotteryDrawRecord record;
    private final List<String> messageList;
    private final List<LuckyGiftProduct> luckyGiftProducts;

    public FishUserDrawer(LuckyGift gift, UserPlus player, SystemConfigFacade config, FishUserJackpotService fishUserJackpotService) {
        this.gift = gift;
        //需要深拷贝
        ...
        this.player = player;
        this.config = config;
        this.fishUserJackpotService = fishUserJackpotService;
    ...
    public LuckyGiftProduct newDraw(List<LuckyGiftProduct> products) {
        ...
        for (LuckyGiftProduct product : products) {
            ...
            boolean isHit = fishUserJackpotService.hit(product.getPrice(), false);
            if (isHit) {
        ...
        fishUserJackpotService.hit(product.getPrice(), true);
        return product;
    }
    ...
    public void sortAndChangeGiftProduct(List<LuckyGiftProduct> products) {
        ...
        jackpotBalance = fishUserJackpotService.getFishUserJackpotBalance();
